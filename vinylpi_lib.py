...
async def main() -> None:
    ...
    if selected_device is None:
        logging.error("No USB audio device found. Exiting.")
        return

    logging.info(f"Using audio device index: {selected_device}")
    ...
    async def recognize_func(audio_data):
        return await recognize_song(audio_data, args.verbose, logging.getLogger())
        ...
        while True:
            try:
                if args.tui:
                    ...
                    logging.info("Starting song detection...")

                song = await check_song_consistency(recognize_func, stream, args.verbose, logging.getLogger())
                ...
                if song and song[0] and song[1] and song[0] != "None" and song[1] != "None":
                    ...
                    if song != current_song:
                        ...
                        if args.tui:
                            ...
                            logging.info(f"Now playing: {title} by {artist}")
                        last_logged_song = log_song_to_lastfm(
                            lastfm_network, artist, title,
                            last_logged_song, logging.getLogger()
                        )
                        current_song = song
                        no_song_detected_count = 0
                    else:
                        consecutive_same_song_count += 1
                        logging.debug(f"Still playing: {title} by {artist}")
                else:
                    consecutive_same_song_count = 0
                    no_song_detected_count += 1
                    if no_song_detected_count >= 3:
                        logging.info("No song detected. Trying aggressive detection...")
                        song = await aggressive_song_check(recognize_func, stream, args.verbose, logging.getLogger())
                        if song and song[0] and song[1] and song[0] != "None" and song[1] != "None":
                            artist, title = song
                            if args.tui:
                                display_tui(song)
                            else:
                                logging.info(f"Detected after aggressive check: {title} by {artist}")
                            last_logged_song = log_song_to_lastfm(
                                lastfm_network, artist, title,
                                last_logged_song, logging.getLogger()
                            )
                            ...
                            if args.tui:
                                ...
                                logging.info("No valid song detected")
                            current_song = None
